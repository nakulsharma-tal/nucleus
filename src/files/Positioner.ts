...
export default class Positioner {
   ...
   */
  public async saveTemporaryFile(
    app: NucleusApp,
    saveString: string,
    fileName: string,
    data: Buffer,
  ...
  public async handleUpload(
    ...
    d(
      `Handling upload (${file.fileName}) for app (${app.slug}) and channel (${channel.name}) for version (${internalVersion.name}) on platform/arch (${file.platform}/${file.arch})`,
    );
   ...
   * to semver.
   */
  public async potentiallyUpdateLatestInstallers(
    lock: PositionerLock,
    app: NucleusApp,
    channel: NucleusChannel,
  ) {
  ...
  private async copyFile(fromKey: string, toKey: string, ref = '') {
    ...
    if (!ref || (await this.store.getFile(refKey)).toString() !== ref) {
      await this.store.putFile(toKey, await this.store.getFile(fromKey), true);
      await this.store.putFile(refKey, Buffer.from(ref), true);
    }
  }

  public updateWin32ReleasesFiles = async (
    lock: PositionerLock,
    app: NucleusApp,
    channel: NucleusChannel,
    arch: string,
  ...
  protected async handleWindowsUpload({ app, channel, file, fileData }: HandlePlatformUploadOpts) {
    ...
    if ((await this.store.putFile(key, fileData)) && file.fileName.endsWith('.nupkg')) {
      ...
      await updateWin32ReleasesFiles({
        app,
        channel,
        arch: file.arch,
        store: this.store,
  ...
  public updateDarwinReleasesFiles = async (
    lock: PositionerLock,
    app: NucleusApp,
    channel: NucleusChannel,
    arch: string,
  ...
  public withLock = async (
    app: NucleusApp,
    fn: (lock: PositionerLock) => Promise<void>,
  ): Promise<boolean> => {
  ...
  public initializeStructure = async (app: NucleusApp, channel: NucleusChannel) => {
    ...
    await this.store.putFile(
      path.posix.join(app.slug, channel.id, 'versions.json'),
