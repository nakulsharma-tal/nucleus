  ...
  describe('/app/:id/channel', () => {
    describe('POST', () => {
      ...
      it('should error if an invalid app ID is provided', async () => {
        const response = await helpers.request.post('/app/10000/channel');
      ...
      it('should error if no name is provided', async () => {
        const response = await helpers.request.post(`/app/${app.id}/channel`);
      ...
      it('should create the channel when a name is provided', async function() {
        ...
        const response = await helpers.request.post(`/app/${app.id}/channel`).send({
          name: 'Stable',
        });
        ...
        expect(
          await helpers.store.hasFile(
            `${app.slug}/${response.body.id}/linux/debian/binary/Release`,
          ),
        ).to.equal(true, 'should create the debian apt repo metadata');

        expect(
          await helpers.store.hasFile(
            `${app.slug}/${response.body.id}/linux/redhat/repodata/repomd.xml`,
          ),
