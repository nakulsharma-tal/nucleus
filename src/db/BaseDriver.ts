...
export abstract class IDBDriver {
  ...
  public abstract renameChannel(
    app: NucleusApp,
    channel: NucleusChannel,
    newName: string,
  ): Promise<NucleusChannel | null>;
  ...
  public abstract getTemporarySaves(
    app: NucleusApp,
    channel: NucleusChannel,
  ): Promise<ITemporarySave[]>;
  public abstract saveTemporaryVersionFiles(
    app: NucleusApp,
    channel: NucleusChannel,
    version: string,
    filenames: string[],
  ...
  public abstract createWebHook(
    app: NucleusApp,
    url: string,
    secret: string,
  ): Promise<NucleusWebHook>;
  ...
  public abstract createWebHookError(
    app: NucleusApp,
    webHookId: number,
    message: string,
    code: number,
  ...
  public abstract setWebHookRegistered(
    app: NucleusApp,
    webHookId: number,
    registered: boolean,
  ): Promise<NucleusWebHook | null>;
  public abstract setVersionDead(
    app: NucleusApp,
    channel: NucleusChannel,
    version: string,
    dead: boolean,
  ): Promise<NucleusChannel>;
  public abstract setVersionRollout(
    app: NucleusApp,
    channel: NucleusChannel,
    version: string,
    rollout: number,
...
export default abstract class BaseDriver extends IDBDriver {
  ...
  protected writeVersionsFileToStore = async (app: NucleusApp, channel: NucleusChannel) => {
    const deepChannel = Object.assign({}, await this.getApp(app.id!)).channels.find(
      (testChannel) => testChannel.id === channel.id,
    );
    if (!deepChannel) return;
    const versionsToWrite = deepChannel.versions;
    await store.putFile(
      path.posix.join(app.slug, channel.id, 'versions.json'),
      Buffer.from(JSON.stringify(versionsToWrite, null, 2)),
      true,
    );
  };
