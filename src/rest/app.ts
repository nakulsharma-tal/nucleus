...
const MAGIC_NAMES = ['__healthcheck', 'public.key'];
  ...
  a(async (req, res) => {
    if (checkField(req, res, 'name')) {
      ...
      if (MAGIC_NAMES.includes(req.body.name)) {
        return res
          .status(400)
          .json({ error: `You can not call your application ${req.body.name}` });
      }
...
const stopNoPerms = (req: Express.Request, res: Express.Response) => {
  if (!hasPermission(req, req.targetApp)) {
    d(
      `A user (${req.user.id}) tried to access an application (${req.targetApp.slug}) that they don't have permission for`,
    );
    res.status(403).json({
      error: 'No Permission',
    });
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['url', 'secret'])) {
      if (
        ...
        req.body.url.startsWith('http://localhost') ||
        req.body.url.startsWith('http://127.0.0.1')
      ) {
...
router.get(
  '/:id/channel/:channelId/temporary_releases',
  requireLogin,
  a(async (req, res) => {
    if (stopNoPerms(req, res)) return;
...
router.get(
  '/:id/channel/:channelId/temporary_releases/:temporarySaveId/:fileName',
  requireLogin,
  a(async (req, res) => {
    if (stopNoPerms(req, res)) return;
    const channel = await driver.getChannel(req.targetApp, req.params.channelId);
    ...
    d(
      `User: ${req.user.id} requested an unencrypted version of a preRelease file: (${req.targetApp.slug}, ${save.version}, ${req.params.fileName})`,
    );
    const positioner = new Positioner(store);
    const data = await positioner.getTemporaryFile(
      req.targetApp,
      save.saveString,
      req.params.fileName,
      save.cipherPassword,
...
router.post(
  '/:id/channel/:channelId/temporary_releases/:temporarySaveId/release',
  requireLogin,
  noPendingMigrations,
  a(async (req, res) => {
    if (stopNoPerms(req, res)) return;
    ...
    if (
      !(await positioner.withLock(req.targetApp, async (lock) => {
        d(
          `User: ${req.user.id} promoted a temporary release for app: '${req.targetApp.slug}' on channel: ${channel.name} becomes version: ${save.version}`,
        );

        storedFileNames = await driver.registerVersionFiles(save);
        d(
          `Tested files: [${save.filenames.join(', ')}] but stored: [${storedFileNames.join(
            ', ',
          )}]`,
        );
        ...
        for (const file of storedFiles) {
          d(
            `Releasing file: ${file.fileName} to version: ${save.version} for (${req.targetApp.slug}/${channel.name})`,
          );
          ...
          const upToDateFile = await driver.storeSHAs(file, generateSHAs(data));
          if (upToDateFile) {
            // Use the Hashed file when handling the upload
            storedVersion.files = storedVersion.files.map((f) =>
              f.id === upToDateFile.id ? upToDateFile : f,
            );
...
router.post(
  '/:id/channel/:channelId/temporary_releases/:temporarySaveId/delete',
  requireLogin,
  noPendingMigrations,
  a(async (req, res) => {
    if (stopNoPerms(req, res)) return;
    ...
    d(
      `User: ${req.user.id} deleted a temporary release for app: '${req.targetApp.slug}' on channel: ${channel.name} would have been version: ${save.version} with ${save.filenames.length} files`,
    );
    const positioner = new Positioner(store);
    if (
      !(await positioner.withLock(req.targetApp, async (lock) => {
...
router.post(
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['version', 'dead'])) {
      ...
      if (isGreatest) {
        d(
          `User: ${req.user.id} tried to make a version (${req.body.version}) as dead=${req.body.dead} for app: '${req.targetApp.slug}' on channel: ${channel.name}.  But was rejected for safety reasons`,
        );
        return res.status(400).json({ error: "You can't kill the latest version" });
      }
      d(
        `User: ${req.user.id} marking a version (${req.body.version}) as dead=${req.body.dead} for app: '${req.targetApp.slug}' on channel: ${channel.name}`,
      );

      const updatedChannel = await driver.setVersionDead(
        req.targetApp,
        channel,
        req.body.version,
        req.body.dead,
...
router.get(
  '/:id/channel/:channelId/invalidate-cache',
  a(async (req, res) => {
    if (stopNoPerms(req, res)) return;
    ...
    const positioner = new Positioner(store);
    await positioner.withLock(req.targetApp, async (lock) => {
      await positioner.potentiallyUpdateLatestInstallers(lock, req.targetApp, channel);
    });
...
router.post(
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['version', 'rollout'])) {
      ...
      d(
        `User: ${req.user.id} changing a version (${req.body.version}) to have a rollout % of '${req.body.rollout}' for app: '${req.targetApp.slug}' on channel: ${channel.name}`,
      );
      const positioner = new Positioner(store);
      const updatedChannel = await driver.setVersionRollout(
        req.targetApp,
        channel,
        req.body.version,
        req.body.rollout,
      ...
      if (updatedVersion) {
        await positioner.withLock(req.targetApp, async (lock) => {
          await positioner.potentiallyUpdateLatestInstallers(lock, req.targetApp, updatedChannel);
        });
      }
  ...
  a(async (req, res) => {
    ...
    if (checkFields(req, res, ['version', 'platform', 'arch'])) {
      ...
      if (['darwin', 'linux', 'win32'].indexOf(req.body.platform) !== -1) {
        ...
        const existingVersion = channel.versions.find(
          (testVersion) => testVersion.name === req.body.version,
        );
        const files = req.files as Express.Multer.File[];
        if (
          existingVersion &&
          existingVersion.files.some(
            (testFile) => files.map((file) => file.originalname).indexOf(testFile.fileName) !== -1,
          )
        ) {
          return res.status(400).json({
        ...
        for (let fileKey = 0; fileKey < files.length; fileKey += 1) {
          ...
          await positioner.saveTemporaryFile(
            req.targetApp,
            temporaryStore.saveString,
            file.originalname,
            file.buffer,
            temporaryStore.cipherPassword,
          );
        }
        res.json({ success: true });
      } else {
